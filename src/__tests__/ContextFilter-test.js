...
describe('when handling legacy user contexts', () => {
  ...
  it('converts non-boolean "anonymous" to boolean "transient"', () => {
    const uf = ContextFilter({ allAttributesPrivate: true });
    expect(uf.filter({ key: 'user', anonymous: 'yes' })).toEqual({ key: 'user', kind: 'user', transient: true });
  });
...
describe('when handling single kind contexts', () => {
  ...
  it('converts non-boolean transient to boolean.', () => {
    const uf = ContextFilter({});
    expect(uf.filter({ kind: 'user', key: 'user', transient: 'string' })).toEqual({
      kind: 'user',
      key: 'user',
      transient: true,
    });

    expect(uf.filter({ kind: 'user', key: 'user', transient: null })).toEqual({
      kind: 'user',
      key: 'user',
      transient: false,
    });
